...
class KFold(_BaseKFold):
    ...
    @_deprecate_positional_args
    def __init__(
        self, n_splits=5, *, shuffle=False, random_state=None, train_subset_frac=1
    ):
        super().__init__(n_splits=n_splits, shuffle=shuffle, random_state=random_state)
        self.train_subset_frac = train_subset_frac
    ...
    def split(self, X, y=None, groups=None):
        for train_index, test_index in super().split(X, y, groups):
            size = int(self.train_subset_frac * len(train_index))
            yield np.random.RandomState(self.random_state).choice(
                train_index, size
            ), test_index
...
class GroupKFold(_BaseKFold):
    ...
    def __init__(self, n_splits=5, train_subset_frac=1):
...
def stratified_subset(a, stratify, size, seed):
    if len(a) == size:
        return a
    train, _ = train_test_split(
        a, train_size=size, stratify=stratify, random_state=seed
    )
    return train
...
class StratifiedKFold(_BaseKFold):
    ...
    * Generate test sets where the smallest and largest differ by at most one
    ...
    def split(self, X, y, groups=None):
        ...
        for train_index, test_index in super().split(X, y, groups):
            size = int(self.train_subset_frac * len(train_index))
            yield stratified_subset(
                train_index,
                stratify=np.asarray(y)[train_index],
                size=size,
